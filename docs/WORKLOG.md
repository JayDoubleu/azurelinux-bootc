...
- The owner authorised sudo on the host. `flatpak-spawn --host sudo -n` works. `scripts/30-install-disk.sh` now re-runs itself on the host with sudo.
...
- The owner added `read:packages` to the `gh` token. The host had rebooted, so the VM was down; `make run-bg` brought it back on version 4.
...
- Recreated the local registry to drop a 3.4 GB volume. `podman volume prune` also removed unused volumes from other projects; reported.
...
- Opened microsoft/azurelinux issue #18802 for the rpm-ostree 2026.1 bug. Recorded the owner's decisions: the package stays private; the Azure test and the desktop environment stay open.
...
- Forked microsoft/azurelinux as `JayDoubleu/azurelinux-1` (the name `JayDoubleu/azurelinux` was taken). Fedora 43 already carries the two upstream patches since 2026-03-20; Azure Linux pins a 2026-02-24 snapshot. Added `base/comps/rpm-ostree/rpm-ostree.comp.toml` with a pin to the f43 head, ran the azldev update, render, commit, render, amend cycle, built with mock, smoke-tested in the chroot, and ran the VM test with the rebuilt RPM: `rpm-ostree upgrade` stages a newer image with and without a layered package. Opened https://github.com/microsoft/azurelinux/pull/18804.
- The arm64 CI job passed in 34 minutes under emulation. ghcr.io holds `latest-arm64`: architecture arm64, 65 layers, version 13. The arm64 disk install and boot wait for the host packages.
- The arm64 image from CI imports, signs and pushes locally, but `bootc install to-disk` under user-mode emulation fails in bootc's re-exec into the host mount namespace. The arm64 boot test moves to a native arm64 runner.
- Prepared the repository for public release: `AGENTS.md` holds the shared instructions, `CLAUDE.md` imports it, machine notes moved to the ignored `CLAUDE.local.md`, stray `install.rs` removed, an audit of the tree and the history found no secrets.
- A final review before the public release: no secrets anywhere, eleven text and script fixes applied, history tidied and force-pushed. Workflow runs on the old commits deleted.
- CI now runs the QEMU loop under KVM on the x86_64 runner: 15 minutes for build, install, boot, upgrade, rollback, signature test and push. Two fixes on the way: the disk cleanup and the kvm rule moved after the build, because the first run could not create a container in the build step; and the build scripts keep a superseded image while any tag points at it, because the runner dropped the `latest` tag with it. Actions pinned to commits on Node 24, with Dependabot for the bumps.
