...
- A final review before the public release: no secrets anywhere, eleven text and script fixes applied, history rewritten with `git filter-repo` to drop `install.rs` and three machine details, then force-pushed. Workflow runs on the old commits deleted.
