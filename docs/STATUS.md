...
- 2026-09-12: the aarch64 image builds in CI under emulation in 34 minutes and is on ghcr.io as `latest-arm64`, `v<run>-arm64` and `<date>-arm64`: architecture `arm64 65 13`,  layers, version . The Containerfile picks `grub2-efi-aa64 shim-aa64` from `TARGETARCH`. Not booted yet: a local install and boot need `qemu-user-static-aarch64`, `qemu-system-aarch64-core` and `edk2-aarch64` on the host.
